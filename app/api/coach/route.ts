...
const NUTRITION_TOOL = {
  ...
  description:
    "Fetch daily nutrition summaries (totals per day) for a date range. Use when the user asks about eating patterns or calorie/protein trends over a period beyond the last 7 days in the snapshot.",
  input_schema: {
    type: "object" as const,
    properties: {
...
const WORKOUT_TOOL = {
  ...
  description:
    "Fetch workout sessions with exercise sets for a date range. Use when the user asks about training progress, volume trends, or specific lifts beyond the last 14 days in the snapshot.",
  input_schema: {
    type: "object" as const,
    properties: {
      start_date: { type: "string", description: "Start date YYYY-MM-DD (inclusive)" },
...
const WEIGHT_TOOL = {
  ...
  description:
    "Fetch body weight log entries for a date range beyond the last 14 days.",
  input_schema: {
    type: "object" as const,
    properties: {
      start_date: { type: "string", description: "Start date YYYY-MM-DD (inclusive)" },
  ...
  if (name === "get_day_meals") {
    ...
    for (const m of meals) {
      if (m.photo_thumb && m.photo_thumb.startsWith("data:")) {
        ...
        const meta = m.photo_thumb.slice(0, commaIdx); // "data:image/jpeg;base64"
  ...
  const recentWorkouts = workouts.slice(0, 8).map((w) => ({
    ...
    exercises: w.exercises
      .map((ex) => ({
        ...
        sets: ex.sets
          .filter((s) => s.type !== "warmup" && (s.reps ?? 0) > 0)
          .map((s) => ({ reps: s.reps, weight_kg: s.weight_kg, type: s.type !== "normal" ? s.type : undefined })),
      }))
      .filter((ex) => ex.sets.length > 0),
  }));
  ...
  return {
    ...
    today: {
      ...
      meals: todayMeals.map((m) => {
        let items: any[] | null = null;
        if ((m as any).items_json) { try { items = JSON.parse((m as any).items_json); } catch {} }
        return {
          description: m.description,
          time: m.created_at,
          calories: m.calories,
          ...
          ...(items && items.length > 0 && {
            items: items.map((it: any) => ({ name: it.name, portion: it.portion, calories: it.calories, protein_g: it.protein_g })),
          }),
        };
      }),
    },
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Agentic loop — model can call tools to fetch deeper data
    let finalReply = "";
